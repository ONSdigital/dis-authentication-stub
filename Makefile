...
.PHONY: build
build: ## Builds binary of application code and stores in bin directory as dis-authentication-stub
	go build -tags 'production' $(LDFLAGS) -o $(BINPATH)/dis-authentication-stub
...
debug: ## Used to run code locally in debug mode
	go build -tags 'debug' $(LDFLAGS) -o $(BINPATH)/dis-authentication-stub
	HUMAN_LOG=1 DEBUG=1 $(BINPATH)/dis-authentication-stub
...
test-component: ## Runs component test suite
	go test -cover -coverpkg=github.com/ONSdigital/dis-authentication-stub/... -component
